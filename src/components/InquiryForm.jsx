 ...
 *    same-origin path. With no such endpoint configured it falls back to the
 *    load-bearing Netlify lead pipeline the site has used in production all
 *    along (see src/components/studio/StudioSections.jsx — the endpoint,
 *    form-name, and field names are LOAD-BEARING; do not rename them). That
 *    post is no-cors and fire-and-forget, exactly as the legacy form's is.
 *
...
// Load-bearing: the outreach pipeline reads this Netlify form. Do not rename.
const LEGACY_FORM_ACTION = "https://whoff-web-studio.netlify.app/";
...
export default function InquiryForm({ initialService = "Website" }) {
  ...
  async function handleSubmit(event) {
    ...
    if (!ENDPOINT) {
      // No same-origin endpoint: deliver through the existing Netlify lead
      // pipeline instead of dropping the inquiry. no-cors means the response
      // is opaque, so a resolved fetch is all the confirmation there is —
      // the same guarantee the legacy form has always given in production.
      try {
        await fetch(LEGACY_FORM_ACTION, {
          method: "POST",
          mode: "no-cors",
  ...
  return (
    <form className="iq-form" onSubmit={handleSubmit} aria-describedby={helpId}>
      ...
      <p className="iq-help" id={helpId}>
        We use these details to respond to your inquiry. By submitting, you
        agree to be contacted by phone, text, or email about your project.
