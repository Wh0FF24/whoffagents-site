        ...
        <div className="space-y-6">
          {posts.map((post, i) => (
            ...
            >
              <Link
                to={`/blog/${post.slug}`}
                className="block group bg-brand-card border border-brand-border rounded-xl p-8 hover:border-brand-cyan/30 transition-all duration-300 hover:-translate-y-0.5 cursor-pointer"
              >
                ...
                <h2 className="text-xl font-bold text-white mb-2 group-hover:text-brand-cyan transition-colors duration-200">
                  {post.title}
                </h2>
                <p className="text-gray-400 text-sm leading-relaxed mb-4">{post.description}</p>
                <span className="inline-flex items-center gap-1.5 text-sm text-brand-cyan group-hover:gap-2.5 transition-all duration-200">
